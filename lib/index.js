 ...
 *   }, root);
 */
export function initWidgetHandler (widgets, appInit, rootElement) {
  if (!widgets) {
        ...
        if (Widget) {
          ...
          const setProps = function ({ event, to, from, payload }) {
            if (event === 'setProps' && to === customData.uuid && from === parentUUID) {
              ReactDOM.render(<Widget {...payload} trigger={trigger}/>, rootElement);
            }
          };
          ...
          ReactDOM.render(<Widget {...customData.props} trigger={trigger}/>, rootElement);
          return;
        } else {
